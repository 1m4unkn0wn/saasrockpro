...
import { Link, useActionData, useNavigate, useOutlet, useSubmit } from "react-router";
...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  await verifyUserHasPermission(request, "admin.relationships.view");
  ...
  const filterableProperties = [
    ...
    },
    {
      ...
      options: [
        ...
        ...(await getAllTenantTypes()).map((item) => {
          return {
            value: item.id,
            name: t(item.title),
          };
  ...
  const { items, pagination } = await getAllTenantRelationshipsWithPagination(filters, currentPagination);
